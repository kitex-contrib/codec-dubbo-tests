...
func TestEchoException(t *testing.T) {
	_, err := cli.EchoException(context.Background(), true)
...
func TestEchoCustomizedException(t *testing.T) {
	_, err := cli.EchoCustomizedException(context.Background(), true)
	exception, ok := hessian2_exception.FromError(err)
	assert.True(t, ok)
